...
resource "aws_lb_listener" "private_link_aws_transcribe" {
  count             = (var.use_private_link && var.use_aws_transcribe_service) ? 1 : 0
  load_balancer_arn = aws_lb.private_link[count.index].arn
  ...
  default_action {
    type             = "forward"
    target_group_arn = aws_lb_target_group.private_link_aws_transcribe[count.index].arn
  }
}

resource "aws_lb_target_group" "private_link_aws_transcribe" {
  count       = (var.use_private_link && var.use_aws_transcribe_service) ? 1 : 0
  name        = "pl-awstrscr-tg-${var.tenant_id}-${var.infrastructure_purpose}"
  port        = var.aws_transcribe_listen_port
  protocol    = "TCP"
  target_type = "alb"
  vpc_id      = aws_vpc.recording_vpc.id
...
resource "aws_lb_target_group_attachment" "private_link_aws_transcribe" {
  count            = (var.use_private_link && var.use_aws_transcribe_service) ? 1 : 0
  target_group_arn = aws_lb_target_group.private_link_aws_transcribe[count.index].arn
  target_id        = aws_lb.recording_load_balancer.arn
  port             = var.aws_transcribe_listen_port
  ...
  depends_on = [
    aws_lb_listener.private_link_aws_transcribe,
    aws_lb.recording_load_balancer,
    aws_lb_target_group.private_link_aws_transcribe
  ]
}
