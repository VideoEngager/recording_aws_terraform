data "aws_ami" "processing_worker_1_ami" {
  most_recent = true
  owners      = ["376474804475"]
...
data "template_file" "processing_worker_1_init" {
  ...
  vars = {
    ...
    log_stream_name       = aws_cloudwatch_log_stream.recsvc_log_stream_processing_unit_1.name
...
resource "aws_instance" "processing_worker_1" {
  ami                  = data.aws_ami.processing_worker_1_ami.id
  instance_type        = var.pn_ec2_type
  subnet_id            = aws_subnet.main-public-1.id
  iam_instance_profile = aws_iam_instance_profile.CloudWatch_Profile.name
  user_data            = data.template_file.processing_worker_1_init.rendered
  ...
  depends_on = [
    ...
    aws_cloudwatch_log_stream.recsvc_log_stream_processing_unit_1,
    aws_efs_mount_target.kurento-worker-1
  ]

  tags = {
    Name        = "ProcessingWorker-1-${var.tenant_id}-${var.infrastructure_purpose}"
